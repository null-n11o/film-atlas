...
映画を入口に歴史・文化・制作・人物の背景を知るWebサービス。Task 1〜7は実装済み。『オデュッセイア』（2026年）の初稿を作成し、中野さんによる内容確認待ち。まず1作品を完成させる。[受入記録](docs/verification/first-work.md)を参照する。
...
[詳細設計](docs/superpowers/specs/2026-09-23-film-atlas-design.md)は2026-09-23に承認済み。[実装計画](docs/superpowers/plans/2026-09-23-film-atlas.md)は同日に承認済み。Task 1〜7はNative実装と独立レビューを完了し、Task 8の初稿を内容確認中。開発要件・設計・実装計画の正本はこのリポジトリに置く。会社の企画・判断・市場調査は01_kcpを参照し、本文を重複管理しない。

## Working rules

- 同じ作業ツリーの書き込み担当は1人。無関係な変更を巻き込まない。
- 上位PLANの提案と、その後の合意を区別する。最初の対象作品・1作品から始める範囲・内容確認担当は受入記録、技術構成は承認済み設計を参照する。
