...
export default defineConfig({output:'static',trailingSlash:'always',publicDir:'.runtime/build/assets-public',integrations:[{name:'reviewed-content-gate',hooks:{
 'astro:config:setup':async({command})=>{if(command==='preview')return;const mode=process.env.FILM_ATLAS_BUILD_MODE;if(!['production','e2e'].includes(mode))throw Error('Use npm run build or npm run dev to validate content first');const manifest=JSON.parse(await readFile('.runtime/build/data/manifest.json','utf8'));if(manifest.mode!==mode)throw Error('CONTENT_MODE_MISMATCH');},
